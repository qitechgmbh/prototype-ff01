...
def deconstruct_and_save(data, output_dir, name, logger):
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    # filename -> list of byte chunks
    groups = {}

    with zipfile.ZipFile(io.BytesIO(data)) as z:
        folders, hours = validate_and_collect(z.namelist())

        logger.write_line(f"Processing hours: {hours}")
        ...
        for hour in sorted(folders):
            for file in folders[hour]:
                ...
                logger.write_line(f"Reading {path}")
                ...
                if not content:
                    logger.write_line(f"Skipping empty file: {path}")
                    continue
    ...
    for file in REQUIRED_FILES:
        ...
        if not out_path.exists():
            logger.write_line(f"Creating empty file: {file}")
    ...
    for file, chunks in groups.items():
        ...
        logger.write_line(f"Writing {file} ({len(chunks)} parts)")
        ...
        logger.write_line(f"Saved {out_path}")
    ...
    logger.write_line(f"Removing orders/tmp: {tmp_dir}")
    shutil.rmtree(order_dir_tmp, ignore_errors=True)

    logger.write_line(f"Orders found {extract_orders(tmp_dir)}")

    final_zip_path = output_dir / "days" / f"{name}.zip"
    create_archive(tmp_dir, final_zip_path)

    logger.write_line(f"Removing days/tmp: {tmp_dir}")
...
def create_archive(tmp_dir, out_path):
    ...
    with zipfile.ZipFile(out_path, "w", compression=zipfile.ZIP_DEFLATED) as z:
        for file_path in sorted(tmp_dir.iterdir()):
            # logger.write_line(f"Adding {file_path.name}")
